...
| `/invite group no-at` | 允许当前群使用 bot，并且不用 @ 也响应 |
| `/invite group ambient [quiet\|balanced\|active]` | 允许当前群使用 bot，不 @ 时先判断是否适合参与 |
| `/invite group history on\|off` | 单独开启或关闭当前群历史记录，默认关闭 |
| `/invite all group` | 允许 bot 所在的所有群使用 |
| `/remove user @某人`, `/remove admin @某人`, `/remove group`, `/remove group no-at`, `/remove group ambient`, `/remove group history` | 移除访问控制条目，或关闭当前群单项策略 |
...
私聊不需要 @。群和话题群默认必须 `@bot`；`@all` 会被忽略。当前群可单独切成 `no-at`（不 @ 都回复）或 `ambient`（不 @ 时由模型先判断是否参与）。支持的云文档评论里 @bot 就会触发回复。
...
| **不用 @ 的群** | 哪些已开放群里的普通消息也会进 bot | `/invite group no-at`（不 @ 都回复）/ `/invite group ambient`（看情况参与） | `/remove group no-at` / `/remove group ambient`（保留群白名单） |
| **历史记录** | 是否持久记录当前群消息，供后续 agent 按需查询 | `/invite group history on` | `/invite group history off` 或 `/remove group history` |
...
- **让某个工作群不用 @ 也能触发** → 在那个群里发 `/invite group no-at`
- **让某个工作群像正常人一样看情况参与** → 在那个群里发 `/invite group ambient`，可加 `quiet` / `balanced` / `active`
- **让 agent 可按需读取这个群的历史上下文** → 在那个群里发 `/invite group history on`；默认不记录历史
- **第一次配，想把 bot 已经在的群一次性全开放** → 发 `/invite all group` 一键拉取 bot 所在的全部群加入名单，之后再用 `/remove group` 删掉不想要的
- **再拉个人一起当管理员** → `/invite admin @他`
- **新建可管理的群** → `/new chat [群名]` 创建飞书群，并让发起人成为飞书原生群主和 bridge 管理员。

### 还需要知道的

- 改完**下一条消息**就生效，不用重启。
- **群里默认要先 @bot 才会回**（私聊不用 @）。在某个已开放群里发 `/invite group no-at` 可以让它不 @ 都回复；发 `/invite group ambient` 可以让它不 @ 时先判断是否该参与；`/config` 里也可以改全局默认。
- **历史记录默认关闭**。只有当前群显式 `/invite group history on` 后，bridge 才会持久记录开启后的新群消息，并在 agent prompt 中暴露 `lark-channel-bridge history tail/search/around` 查询方式。agent 不是自动看到所有群消息；只有任务需要上下文时，才会按需查询开启 history 后记录到的消息。
...
`allowedUsers` / `admins` 填用户 `open_id`，`allowedChats` 和 `chatPolicies` 填群 `chat_id`。优先用 `/invite group no-at`，bridge 会自动使用当前群真实 ID，不用手动复制。手动找 ID 最简单的办法：让对方给 bot 发条消息（群里就 @ 它一下），然后看当前 profile 的日志：
