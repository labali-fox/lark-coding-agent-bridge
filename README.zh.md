...
| `/remove user @某人`, `/remove admin @某人`, `/remove group`, `/remove group no-at` | 移除访问控制条目，或关闭当前群不用 @ 的覆盖 |
...
| **不用 @ 的群** | 哪些已开放群里的普通消息也会进 bot | `/invite group no-at`（当前群） | `/remove group no-at`（保留群白名单） |
...
- 改完**下一条消息**就生效，不用重启。
- **群里默认要先 @bot 才会回**（私聊不用 @）。在某个已开放群里发 `/invite group no-at` 可以只覆盖当前群；`/config` 里也可以改全局默认。
